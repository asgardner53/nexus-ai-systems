# Nexus AI Studio Register

## Status

Approved live register

## Owner

Alec Gardner

## Purpose

This register records the approved status, classification, ownership and authoritative repository location of Nexus AI Studios.

| Studio | Identifier | Classification | Owner | Status | Effective date | Next review | Authoritative repository | Control record |
|---|---|---|---|---|---|---|---|---|
| Assessment Review Studio | NEX-AI-ARS-001 | Restricted | Alec Gardner | Approved | 26 July 2026 | 26 July 2027 | `asgardner53/nexus-ai-systems` | `studio-control-records/assessment-review-studio-v1.0.md` |
| AIRBOK Development Studio | NEX-AI-AIRBOK-001 | Controlled | Alec Gardner | Approved | 26 July 2026 | 26 July 2027 | `asgardner53/airbok-foundational-edition` with governance record in `asgardner53/nexus-ai-systems` | `studio-control-records/airbok-development-studio-v1.0.md` |
| LinkedIn Thought Leadership Studio | NEX-AI-LTL-001 | Controlled | Alec Gardner | Approved | 26 July 2026 | 26 July 2027 | `asgardner53/nexus-ai-systems` | `studio-control-records/linkedin-thought-leadership-studio-v1.0.md` |
| Nexus Ebook Studio | NEX-AI-EBS-001 | Controlled | Alec Gardner | Approved | 26 July 2026 | 26 July 2027 | `asgardner53/nexus-ai-systems` with dedicated title repositories or controlled folders as approved | `studio-control-records/nexus-ebook-studio-v1.0.md` |
| Nexus Masterclass Studio | NEX-AI-MCS-001 | Professional | Alec Gardner | Approved | 26 July 2026 | 26 July 2027 | `asgardner53/Nexus-BMG-masterclass-studio` with governance record in `asgardner53/nexus-ai-systems` | `studio-control-records/nexus-masterclass-studio-v1.0.md` |
| ChatGPT Studio Development Guide | NEX-AI-CSD-001 | Professional | Alec Gardner | Approved | 26 July 2026 | 26 July 2027 | `asgardner53/nexus-ai-systems` | `studio-control-records/chatgpt-studio-development-guide-v1.0.md` |
| Grants Access Studio | NEX-AI-GAS-001 | Pending owner classification | Alec Gardner | Charter approved — operational release pending pilot | 22 September 2026 | Before pilot release | `asgardner53/nexus-ai-systems` | `studio-control-records/grants-access-studio-charter-v1.0.md` |

## Register rules

1. A Studio must not be described as governed or approved unless its control record is approved and linked here.
2. Material changes to purpose, classification, ownership, connectors or authority require a register update.
3. Retired and superseded Studios remain listed with their final status and archive location.
4. Student evidence, client confidential material and other restricted operational records must not be added to this register.

**AI assists. Humans decide. GitHub records.**
